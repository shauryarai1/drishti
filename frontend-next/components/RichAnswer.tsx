 ...
 * lists, fenced code). Everything is built as React elements - React's unsafe
 * raw-HTML injection prop is deliberately never used anywhere, so markup in a
 * model response can never execute and raw formatting tokens never reach the
      ...
      {blocks.map((block, index) => {
        if (block.type === 'code') {
          return (
            <pre
              key={index}
              className="mt-2.5 overflow-x-auto rounded-lg border border-[#A62A34]/25 bg-[#0D0607] px-3 py-2 text-[13px] leading-relaxed text-[#EEE9DF]/90"
            >
